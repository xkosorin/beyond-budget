...
const Budget = ({ budget }: Props) => {
  const value = Math.floor(
    ((parseFloat(budget.size) - parseFloat(budget.spent)) /
      parseFloat(budget.size)) *
      100,
  );
  const remaining = parseFloat(budget.size) - parseFloat(budget.spent);
    ...
    <div className="flex w-full min-w-[80px] flex-col items-start justify-between whitespace-nowrap py-2 text-sm font-bold md:text-base xl:min-w-[120px]">
      <div className="flex w-full flex-row justify-between">
        ...
        <span className="self-end">
          {remaining.toFixed(2)} / {budget.size} €
        </span>
      </div>
      <Progress
        value={value}
